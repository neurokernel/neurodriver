...
class DopamineLeakyIAF(BaseAxonHillockModel):
    updates = ['dopamine_spike_state', 'V']
    accesses = ['I']
    ...
    def __init__(self, params_dict, access_buffers, dt,
        ...
        dtypes.update({k: self.dtype if not k == 'dopamine_spike_state' else np.int32 for k in self.updates})
...
__global__ void update(int num_comps, %(dt)s dt, int nsteps,
               ...
               %(dopamine_spike_state)s* g_spike_state, %(V)s* g_V)
{
...
if __name__ == '__main__':
    ...
    fl_output_processor = FileOutputProcessor([('dopamine_spike_state', None),('V', None)], 'new_output.h5', sample_interval=1)
