...
# per neuron
cuda_src = """
__global__ void dopamine_modulated_alpha_synapse(
    int num,
    %(type)s dt,
    int *spike,
    int ld,
    ...
    int* delayb)
{
    ...
    for( int i=tid; i<num; i+=tot_threads ){
        ...
        if(npre[i]){
            ...
            a1[i] = new_a[1];
            a2[i] = new_a[2];
            cond[i] = fmax( 0., new_a[0]*gmax - 1.0 * dopamine[i]);
        }
        if(npreb[i]){
			col = currentb-delayb[i];
            if(col < 0)
            {
                col = buffer_lengthb + col;
            }
            pre = col*ldb + Preb[cumpreb[i]]; 
            if( spikeb[pre] )
                dopamine[i] += 1.0 * fmax( 0., new_a[0]*gmax);
		}
        dopamine[i] = dopamine[i] - 0.0002 * dopamine[i]; // Dopamine decay
}
	
    return;
}
"""
class DopamineModulatedAlphaSynapse(BaseSynapseModel):
    accesses = ['spike_state', 'dopamine_spike_state']
    ...
    def __init__( self, params_dict, access_buffers, dt,
        ...
        self.dopamine   = garray.zeros( (self.num,), dtype=np.float64 )
    ...
    def run_step(self, update_pointers, st = None):
        self.update.prepared_async_call(
			...
			self.access_buffers['dopamine_spike_state'].gpudata,
            self.access_buffers['dopamine_spike_state'].ld,
            self.access_buffers['dopamine_spike_state'].current,
            self.access_buffers['dopamine_spike_state'].buffer_length,
			...
			self.params_dict['pre']['dopamine_spike_state'].gpudata,
            self.params_dict['npre']['dopamine_spike_state'].gpudata,
            self.params_dict['cumpre']['dopamine_spike_state'].gpudata,
            self.params_dict['conn_data']['dopamine_spike_state']['delay'].gpudata
			)
    ...
    def get_gpu_kernel(self, dtype=np.double):
        ...
        func = mod.get_function("dopamine_modulated_alpha_synapse")
        func.prepare('idPiiiPiiiPPPPPPPPPPPPPPPP')
        return func
